(ns tonline.models.updatedb
    (:require [clojure.java.jdbc :as sql]
              [tonline.models.db :as db]
...
(defn insert-from-tosmp [kodepel]
  (let [data (get-data1 (str "select * from bankproset where kodepel='" kodepel "' and jumpil <> ''") 2)
        cdt (count data)]
    (loop [i 0]
      (if (= i cdt)
        "selesai"
        (do
          (db/insert-data "proset"
                          {:kode (:kode (nth data i))
                           :kodekategori kodepel
                           :jsoal (:jsoal (nth data i))
                           :waktu (:waktu (nth data i))
                           :keterangan (:keterangan (nth data i))
                           :status (:status (nth data i))
                           :kunci (str (vec (map #(str %) (vec (:kunci (nth data i))))))
                           :jenis (str (vec (map #(str %) (vec (:jenis (nth data i))))))
                           :upto (str (vec (repeat (:jsoal (nth data i)) (:jumpil (nth data i)))))
                           :acak (:acak (nth data i))
                           :skala (:skala (nth data i))
                           :nbenar (:nbenar (nth data i))
                           :nsalah (:nsalah (nth data i))
                           :jumpil (read-string (:jumpil (nth data i)))
                           :golongan "1"
                           :polaacak (str (vec (range 1 (inc (:jsoal (nth data i))))))
                           :kodex (:kodex (nth data i))
                           :justonce "1"
                           :pretext (str (vec (map #(str %) (read-string (:pretext (nth data i))))))
                           :sound (str (vec (map #(str %) (read-string (:sound (nth data i))))))
                           })
          (recur (inc i)))))))
